...
- (void)_commonSetup
{
   ...
   _captureDevice   = [AVCaptureDevice defaultDeviceWithMediaType:AVMediaTypeVideo];  // [AVCaptureDevice device];
   _videoDataOutput = [[AVCaptureVideoDataOutput alloc] init];  // was [AVCaptureVideoDataOutput output];

   _captureSessionQueue = dispatch_queue_create ("com.example.apple-samplecode.CaptureSessionQueue", NULL);
   _videoDataOutputQueue = dispatch_queue_create ("com.example.apple-samplecode.VideoDataOutputQueue", NULL);
   ...
   // self.bottomToTopTransform = CGAffineTransformScale (CGAffineTransformMakeTranslation(0., -1.), 1., -1.);
       ...
       withTransitionCoordinator:(id <UIViewControllerTransitionCoordinator>)coordinator
{
   ...
   // Alt: UIInterfaceOrientation orientation = [[UIApplication sharedApplication] statusBarOrientation];
...
- (void)calculateRegionOfInterest
{
   ...
   if ((self.currentDeviceOrientation == UIDeviceOrientationPortrait) ||
       (self.currentDeviceOrientation == UIDeviceOrientationUnknown))  {
      size.width  = fmin (desiredWidthRatio * self.bufferAspectRatio, maxPortraitWidth);
      size.height = desiredHeightRatio / self.bufferAspectRatio;
   }
   else  {
      size.width  = desiredWidthRatio;
      size.height = desiredHeightRatio;
   }
...
- (void)setupOrientationAndTransform  // called from -calculateRegionOfInterest
{
   ...
   self.visionToAVFTransform = CGAffineTransformConcat (self.roiToGlobalTransform, self.bottomToTopTransform);
   self.visionToAVFTransform = CGAffineTransformConcat (self.visionToAVFTransform, self.uiRotationTransform);
}
...
- (void)setupCamera
{
   ...
   AVCaptureDeviceDiscoverySession  *captureDeviceDiscoverySession = nil;
   
   captureDeviceDiscoverySession = [AVCaptureDeviceDiscoverySession discoverySessionWithDeviceTypes:types
                                         mediaType:AVMediaTypeVideo
                                          position:AVCaptureDevicePositionBack];
   
   if (!captureDeviceDiscoverySession || !captureDeviceDiscoverySession.devices.count)  {
      NSLog (@"Could not create capture device - no devices.");
      return;
   }
      
   
   self.captureDevice = [captureDeviceDiscoverySession.devices objectAtIndex:0];  // 2
   ...
   [self.videoDataOutput setSampleBufferDelegate:(id)self 
                                           queue:self.videoDataOutputQueue];
   
   NSString  *key = (NSString*)kCVPixelBufferPixelFormatTypeKey;
   NSNumber  *value = [NSNumber numberWithUnsignedInt:kCVPixelFormatType_420YpCbCr8BiPlanarFullRange];
   
   self.videoDataOutput.videoSettings = [NSDictionary dictionaryWithObject:value
                                                                    forKey:key];
   // videoDataOutput.videoSettings = @[kCVPixelBufferPixelFormatTypeKey:kCVPixelFormatType_420YpCbCr8BiPlanarFullRange];
   ...
   if ([self.captureDevice lockForConfiguration:&error])  {
      ...
      NSLog (@"Could not set zoom level due to error: %@", error);
      return;
   }
